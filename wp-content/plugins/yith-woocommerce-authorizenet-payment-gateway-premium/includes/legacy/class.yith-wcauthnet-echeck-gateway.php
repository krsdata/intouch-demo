...
if ( ! class_exists( 'YITH_WCAUTHNET_eCheck_Gateway' ) ) {
	...
	class YITH_WCAUTHNET_eCheck_Gateway extends WC_Payment_Gateway {
		...
		public function __construct() {
			...
			$this->method_title       = apply_filters( 'yith_wcauthnet_echeck_method_title', __( 'Authorize.net eCheck', 'yith-wcauthnet' ) );
			$this->method_description = apply_filters( 'yith_wcauthnet_echeck_method_description', __( 'Pay with Authorize.net', 'yith-wcauthnet' ) );
			...
			$this->order_button_text  = apply_filters( 'yith_wcauthnet_echeck_order_button_text', $this->get_option( 'order_button', __( 'Proceed to Authorize.net', 'yith-wcauthnet' ) ) );
		...
		public function init_form_fields() {
			$this->form_fields = apply_filters( 'yith_wcauthnet_credit_card_gateway_options', array(
				'enabled'         => array(
					...
					'type'    => 'checkbox',
					'label'   => __( 'Enable Authorize.net eCheck Payment', 'yith-wcauthnet' ),
					'default' => 'no'
				),
				'title'           => array(
					...
					'default'     => __( 'eCheck on Authorize.net', 'yith-wcauthnet' ),
					'desc_tip'    => true,
				),
				'description'     => array(
				...
				'login_id'        => array(
					...
					'type'        => 'text',
					'description' => __( 'Univocal ID login associated to the account of the admin (it can be recovered in the "API Login ID and Transaction Key" section). If no detail is stated, the system will try to use the parameter of the "Authorize.net Credit Card" payment gateway.', 'yith-wcauthnet' )
				),
				'transaction_key' => array(
					'title'       => __( 'Transaction Key', 'yith-wcauthnet' ),
					'type'        => 'text',
					'description' => __( 'A unique key used to validate requests to Authorize.net (it can be recovered in the "API Login ID and Transaction Key" section). If no detail is stated, the system will try to use the parameter of the "Authorize.net Credit Card" payment gateway.', 'yith-wcauthnet' )
				),
				'md5_hash'        => array(
					'title'       => __( 'Md5 Hash', 'yith-wcauthnet' ),
					'type'        => 'text',
					'description' => __( 'A unique key used to validate the answers from Authorize.net (it can be set in the " MD5 Hash " section). If no detail is stated, the system will try to use the parameter of the "Authorize.net Credit Card" payment gateway.', 'yith-wcauthnet' )
				),
				'itemized'        => array(
					'title'       => __( 'Enable itemized transaction', 'yith-wcauthnet' ),
					'type'        => 'checkbox',
					'description' => __( 'Enable the registration of the items in the cart during the transaction (up to a maximum of 30 items)', 'yith-wcauthnet' ),
					'default'     => 'yes'
				),
				'sandbox'         => array(
				...
				<div class="yith-wcauthnet-echeck-form">
					<p class="form-row form-row-first">
						<label for="yith-wcauthnet-routing-number">
							<?php _e( "Bank Routing Number", 'yith-wcauthnet' ); ?>
					...
					<p class="form-row form-row-last">
						<label for="authorize-net-cim-account-number">
							<?php _e( "Bank Account Number", 'yith-wcauthnet' ); ?>
							<span class="required">*</span>
						</label>
		...
		public function admin_notices() {
			if ( empty( $this->login_id ) || empty( $this->transaction_key ) ) {
				echo '<div class="error"><p>' . __( 'Please enter Login ID and Transaction Key for Authorize.net eCheck gateway.', 'yith-wcauthnet' ) . '</p></div>';
			}
		}
		...
		public function process_payment( $order_id ) {
			...
			if( 'echeck' == $payment->type && ( empty( $payment->routing_number ) || empty( $payment->account_number ) ) ){
				wc_add_notice( __( 'Missing required information', 'yith-wcauthnet' ), 'error' );
				return;
			}
			...
			if ( ! empty( $response ) ) {
					...
					if ( 1 == $transaction_status ) {
						if ( $order->has_status( 'completed' ) ) {
							...
							if ( $order->get_total() != $transaction_amount ) {
								...
								$order->update_status( 'on-hold', sprintf( __( 'Validation error: Authorize.net amounts do not match with (%s).', 'yith-wcauthnet' ), $transaction_amount ) );

								wc_add_notice( sprintf( __( 'Validation error: Authorize.net amounts do not match with (%s).', 'yith-wcauthnet' ), $transaction_amount ), 'error' );
							...
							if ( strcasecmp( trim( $order->billing_email ), trim( $transaction_email ) ) != 0 ) {
								...
								// Put this order on-hold for manual checking
								$order->update_status( 'on-hold', sprintf( __( 'Validation error: Authorize.net responses from a different email address than (%s).', 'yith-wcauthnet' ), $transaction_email ) );

								wc_add_notice( sprintf( __( 'Validation error: Authorize.net responses from a different email address than (%s).', 'yith-wcauthnet' ), $transaction_email ), 'error' );
							...
							// Mark as complete
							$order->add_order_note( sprintf( __( 'Authorize.net payment completed (message: %s). Transaction ID: %s', 'yith-wcauthnet' ), $transaction_message, $transaction_id ) );
						...
						wc_add_notice( sprintf( __( 'Payment error: %s', 'yith-wcauthnet' ), $response_message ), 'error' );
				...
				wc_add_notice( __( 'An error occurred while processing the payment; please try again later', 'yith-wcauthnet' ), 'error' );
		...
		public function process_refund( $order_id, $amount = null, $reason = '' ) {
			...
			if( ! $order || ! $this->can_refund_order( $order ) ){
				...
				$order->add_order_note( __( 'Authorize.net refund failed: No transaction ID', 'yith-wcauthnet' ) );
				return false;
			}
			...
			if( ! empty( $response ) ){
				...
				if( 1 == $transaction_status ){
					...
					$order->add_order_note( __( 'Authorize.net refund approved', 'yith-wcauthnet' ) );
				}
				else{
					$response_message = '';
					...
					$this->log->add( 'authorize.net', sprintf( __( 'Refund error: %s', 'yith-wcauthnet' ), $response_message ) );
					$order->add_order_note( sprintf( __( 'Refund error: %s', 'yith-wcauthnet' ), $response_message ) );
					return false;
				}
			}
			else{
				$this->log->add( 'authorize.net', __( 'Refund error: unknown error', 'yith-wcauthnet' ) );
				$order->add_order_note( __( 'Refund error: unknown error', 'yith-wcauthnet' ) );
				return false;
			}
