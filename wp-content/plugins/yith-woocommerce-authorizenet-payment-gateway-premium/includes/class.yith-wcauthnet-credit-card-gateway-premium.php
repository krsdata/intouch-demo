	...
	class YITH_WCAUTHNET_Credit_Card_Gateway_Premium extends YITH_WCAUTHNET_Credit_Card_Gateway {
		...
		public function __construct() {
			...
			if( 'redirect' == $this->payment_method ){
				add_action( 'woocommerce_receipt_yith_wcauthnet_gateway', array( $this, 'print_authorize_net_payment_form' ), 10, 1 );
			}
		}
		...
		public function add_advanced_options() {
			...
			$premium_options = array(
				'payment_method' => array(
					'title' => __( 'Request mode', 'yith-wcauthnet' ),
					'type' => 'select',
					'description' => __( 'Selecting "Redirect", users will be able to complete the payment on the pages of Authorize.net; on the contrary, the option "API" will allow using the potentialities of the APIs to complete the purchase without external redirects.', 'yith-wcauthnet' ),
					'options' => array(
						'redirect' => __( 'Redirect', 'yith-wcauthnet' ),
						'api' => __( 'API', 'yith-wcauthnet' )
					),
					'default' => 'api'
				),
				'cim_handling' => array(
					'title' => __( 'Enable Customer Information Manager (CIM)', 'yith-wcauthnet' ),
					'type' => 'checkbox',
					'description' => __( 'It allows the system to store the payment methods of the users, in order to use them in the future without the need to write the details again.', 'yith-wcauthnet' ),
					'default' => 'yes'
				),
				'itemized' => array(
			...
			if( ! empty( $user_id ) && $this->cim_handling == 'yes' ){
				...
				if( ! empty( $user_profile ) && empty( $payment_profile ) ){
					...
					if( isset( $response->customerPaymentProfileId ) && $payment->save_method ){
						...
						$new_payment_profiles = $registered_payment_profiles + array( $payment_profile_id => array(
		...
		protected function _get_payment_details(){
			...
			if( $card_id && $card_id != 'new' ){
				...
				$payment_obj->card_number = str_replace( array( ' ', '-' ), '', $_POST[ $this->id . '-card-number' ] );
				$payment_obj->expiration_date = str_replace( array( '/', ' ' ), '', $_POST[ $this->id . '-card-expiry' ]);
